...
**Total Documents**: 45+ Core Markdown files (460+ total .md files in project)  
**Purpose**: Complete index of essential project documentation

**🆕 NEW (Nov 21):** Dashboard data conversion fix! Wind farm mapping complete!
...
### 🚀 Latest Updates (21 Nov 2025)
0. **[DASHBOARD_FIX_NOV_21_2025.md](#dashboard-fix-nov-21)** - 🔧 **Dashboard MW→GW conversion fix** (NEW!)
1. **[WIND_FARM_MAPPING_COMPLETE.md](#wind-farm-mapping)** - 🌬️ **43 offshore + 414 onshore wind farms mapped** (NEW!)
2. **[comprehensive_dashboard_update.py](#comprehensive-dashboard-update)** - 📊 **Complete dashboard updater** (NEW!)
3. **[STOP_DATA_ARCHITECTURE_REFERENCE.md](#stop-architecture)** - ⚠️ **Updated with MW units warning** (UPDATED!)

### 🚀 Recent Updates (18 Nov 2025)
...
**🆕 November 21 Update**: Added critical warning about `bmrs_fuelinst_iris.generation` column being in **MW** (not MWh). Many dashboard update failures were caused by incorrect unit assumptions.

**Key Content**:
- **Golden Rules**: Check table coverage FIRST, Know your two architectures, Use UNION patterns
- **Table Coverage Matrix**: Which tables have which date ranges + **UNITS** column added
- **Data Type Fixes**: How to handle DATETIME vs STRING (Cast to DATE)
- **⚠️ NEW: Unit Conversion Warning**: `generation` in MW, NOT MWh → `gw = mw / 1000` (NOT `/ 500`)
...
- `bmrs_fuelinst_iris`: **generation column is in MW** ⚠️ (NOT MWh!)
...
**Reference Scripts** with correct MW→GW conversion:
- ✅ `update_dashboard_preserve_layout.py` (line 75)
- ✅ `update_dashboard_enhanced.py` (line 75)
- ✅ `comprehensive_dashboard_update.py` (NEW)

---

### DASHBOARD_FIX_NOV_21_2025.md {#dashboard-fix-nov-21}
**Category**: 🔧 Dashboard  
**Status**: ✅ RESOLVED  
...
**Summary**:  
Complete resolution of dashboard data display issues. Dashboard was showing generation values in thousands of GW (e.g., 2695 GW for CCGT) instead of realistic values (~20 GW). Root cause was incorrect unit conversion.

**Problem**:
- Total Generation showing 2695 GW (should be ~35 GW)
- Individual fuel types showing hundreds of GW
- Timestamp not updating
- Fuel breakdown frozen
- Interconnector flags missing
- Outages not displaying

**Root Cause**:
```python
# ❌ WRONG - Treating generation as MWh per settlement period
generation_gw = generation_mwh / 500  # Incorrect!

# ✅ CORRECT - generation is already in MW
generation_gw = generation_mw / 1000  # Simple MW to GW
```

**Solution**:
Created `comprehensive_dashboard_update.py` that correctly:
1. Queries `bmrs_fuelinst_iris` (generation in MW)
2. Converts MW → GW (`/ 1000`)
3. Updates all dashboard sections:
   - Timestamp & freshness
   - System metrics (Total Gen: 35.4 GW ✅)
   - Fuel breakdown (10 types with emojis)
   - Interconnectors (10 with country flags 🇫🇷🇮🇪🇧🇪🇳🇱🇳🇴🇩🇰)
   - Outages (10 with progress bars 🟥⬜)

**Test Results**:
```
✅ Settlement Period: SP23
✅ Total Generation: 35.4 GW (was 2695 GW)
✅ Market Price: £121.64/MWh
✅ All 10 fuel types updated
✅ All 10 interconnectors with flags
✅ All 10 outages with emojis
```

**Documentation Updates**:
- STOP_DATA_ARCHITECTURE_REFERENCE.md → Added MW units warning
- DASHBOARD_CURRENT_STATUS_NOV_20_2025.md → Added "CRITICAL FIX" section
...
### WIND_FARM_MAPPING_COMPLETE.md {#wind-farm-mapping}
...
**Summary**:  
Comprehensive UK wind generation mapping project completed. Imported 43 offshore wind farms from Wikipedia, mapped 414 onshore wind farms from DNO data, and created interactive visualization showing all UK wind capacity with GSP connections.

**Achievements**:
1. **Offshore Wind Import**: 43 farms, 16,410 MW, 2,869 turbines
2. **Onshore Wind Mapping**: 414 farms >= 1 MW, 6,677 MW
3. **BigQuery Table**: New `offshore_wind_farms` table with full metadata
4. **Interactive Map**: Folium map with toggleable layers
5. **Dashboard Integration**: Wind map added to Dashboard row 100
6. **DNO API Discovery**: Explored all 6 UK DNOs (2 active APIs, 4 manual)

**Data Sources**:
- Wikipedia: UK offshore wind farms (July 2025 data)
- BigQuery `all_generators`: DNO embedded capacity registers
- BigQuery `bmu_registration_data`: Balancing Mechanism units

**Key Findings**:
- **70.6% offshore**: 16,016 MW offshore vs 6,677 MW onshore
- **Scotland leads**: 1,581 MW wind in Scottish GSP zones
- **10 farms missing** from BMU: Gwynt y Môr (576 MW) + 9 others (1,444 MW total)
- **Offshore average**: 391 MW per farm
- **Onshore average**: 16 MW per farm (24x smaller)

**Technical Details**:
- Coordinate conversion: British National Grid (EPSG:27700) → WGS84 (EPSG:4326)
- Library: pyproj for coordinate transformation
- GeoJSON: 333 GSP zones colored by wind capacity
- Interactive features: Click for details, toggleable layers, zoom/pan

**Files Created**:
1. `import_offshore_wind_farms.py` - Wikipedia → BigQuery import
2. `explore_dno_apis.py` - DNO API discovery tool
3. `create_wind_farm_maps.py` - Wind map generator (500+ lines)
4. `map_wind_capacity.html` - Interactive map
5. `map_wind_capacity.png` - Dashboard image (1400x900)
...
### comprehensive_dashboard_update.py {#comprehensive-dashboard-update}
...
**Summary**:  
Complete dashboard updater that correctly handles ALL sections with proper data conversions. Replaces multiple partial update scripts with single comprehensive solution.

**Features**:
- ✅ Correct MW → GW conversion (`/ 1000`)
- ✅ Latest publishTime query pattern (not settlementPeriod)
- ✅ Market price from `bmrs_mid_iris` (`dataProvider = 'APXMIDP'`)
- ✅ Interconnector flows with country flags
- ✅ Power station outages with emojis and progress bars
- ✅ Timestamp and freshness indicators

**Updates These Dashboard Sections**:
- Row 2: Timestamp `⏰ Last Updated: YYYY-MM-DD HH:MM:SS | ✅ FRESH`
- Row 3: Data freshness legend
- Row 4: System metrics (Total Gen, Supply, Renewables %, Market Price)
- Rows 6-15: Fuel breakdown (10 types: 🔥 CCGT, 💨 WIND, ⚛️ NUCLEAR, etc.)
- Rows 7-16: Interconnectors (10 with flags: 🇫🇷 🇮🇪 🇧🇪 🇳🇱 🇳🇴 🇩🇰)
- Rows 23-32: Outages (10 with progress bars: 🟥🟥🟥⬜⬜)

**Correct Query Patterns**:
```python
# Generation (latest publishTime, not settlementPeriod)
WITH latest_data AS (
    SELECT fuelType, generation, publishTime
    FROM bmrs_fuelinst_iris
    WHERE DATE(settlementDate) = CURRENT_DATE()
    ORDER BY publishTime DESC LIMIT 1000
),
current_sp AS (
    SELECT MAX(publishTime) as latest_time
    FROM latest_data
)
SELECT fuelType, ROUND(SUM(generation), 1) as total_mw
FROM latest_data
WHERE publishTime = (SELECT latest_time FROM current_sp)
GROUP BY fuelType

# Convert: generation_gw = total_mw / 1000.0
```
