...
# Use bmrs_costs for system prices (has systemSellPrice/systemBuyPrice)
TABLE_FR_PRICES = f"{GCP_PROJECT_ID}.{BQ_DATASET}.fr_clearing_prices"
TABLE_BM_BOAS = f"{GCP_PROJECT_ID}.{BQ_DATASET}.bmrs_boalf"
TABLE_SYSTEM_PRICES = f"{GCP_PROJECT_ID}.{BQ_DATASET}.bmrs_costs"
...
# ---- NESO / BMRS API Endpoints ----
# Using existing BigQuery tables instead of live API calls for now
# Real-time APIs can be added later
NESO_FR_API = "https://data.neso.energy/api/3/action/datastore_search"
NESO_API_KEY = os.environ.get("NESO_API_KEY", "")

# BMRS replacement API (Elexon)
BMRS_BASE_API = "https://data.elexon.co.uk/bmrs/api/v1"
...
def ingest_neso_fr(start_date: date, end_date: date) -> pd.DataFrame:
    """
    Fetch DC/DM/DR prices from BigQuery (using existing fr_clearing_prices table)
    """
    logging.info(f"Fetching FR prices from BigQuery {start_date} to {end_date}...")
    
    client = make_bq_client()
    
    # Use existing FR optimizer table with correct schema
    query = f"""
    SELECT 
        efa_date as date,
        efa_block,
        service,
        clearing_price_gbp_per_mw_h as clearing_price,
        block_start,
        block_end
    FROM `{GCP_PROJECT_ID}.{BQ_DATASET}.fr_clearing_prices`
    ...
    try:
        ...
        logging.info(f"✅ Loaded {len(df)} FR price rows from BigQuery")
        return df
    except Exception as e:
        logging.warning(f"❌ FR data fetch error: {e}")
        return pd.DataFrame()
...
def ingest_bmrs_system_prices(start_date: date, end_date: date) -> pd.DataFrame:
    """
    Fetch system prices (SSP/SBP) from existing BigQuery tables (bmrs_costs)
    """
    logging.info(f"Fetching system prices from BigQuery {start_date} to {end_date}...")
    ...
    FROM `{GCP_PROJECT_ID}.{BQ_DATASET}.bmrs_costs`
    ...
    try:
        df = client.query(query, job_config=job_config).to_dataframe()
        logging.info(f"✅ Loaded {len(df)} system price rows from BigQuery")
        return df
    except Exception as e:
        logging.warning(f"❌ System prices fetch error: {e}")
        return pd.DataFrame()


def ingest_bmrs_boas(start_date: date, end_date: date) -> pd.DataFrame:
    """
    Fetch BOA data (bid/offer acceptances) from BigQuery (bmrs_boalf table)
    """
    logging.info(f"Fetching BOAs from BigQuery {start_date} to {end_date}...")
    
    client = make_bq_client()
    
    query = f"""
    SELECT 
        TIMESTAMP(acceptanceTime) as timestamp,
        bmUnit as unit_id,
        levelFrom as bid_volume,
        levelTo as offer_volume,
        acceptanceNumber as acceptance_number,
        acceptanceTime
    FROM `{GCP_PROJECT_ID}.{BQ_DATASET}.bmrs_boalf`
    WHERE settlementDate BETWEEN @start_date AND @end_date
    ...
    try:
        df = client.query(query, job_config=job_config).to_dataframe()
        logging.info(f"✅ Loaded {len(df)} BOA rows from BigQuery")
        
        # Add mock acceptance price (real calculation would use bmrs_bod)
        if not df.empty:
            df['acceptance_price'] = 50.0  # placeholder
            df['cost'] = 0.0
        
        return df
    except Exception as e:
        logging.warning(f"❌ BOA fetch error: {e}")
        return pd.DataFrame()
...
class FRBMOptimiser:
    ...
    def _fetch_fr_prices(self, start_date: date, end_date: date) -> pd.DataFrame:
        ...
        WHERE efa_date BETWEEN @s AND @e
        """
        job = self.bq.query(
            q,
            job_config=bigquery.QueryJobConfig(
    ...
    def fetch_system_prices(self, start_date: date, end_date: date) -> pd.DataFrame:
        ...
        WHERE settlementDate BETWEEN @s AND @e
            AND systemSellPrice IS NOT NULL
            AND systemBuyPrice IS NOT NULL
        """
        job = self.bq.query(
    ...
    def optimise_fr_vs_bm(
        ...
        for _, row in fr_pivot.iterrows():
            ...
            block_start = datetime.combine(d, datetime.min.time(), tzinfo=timezone.utc) + timedelta(hours=(efa - 1) * 4)
            ...
            for svc in services:
                if svc in ["DC", "DM", "DR"]:
                    ...
                    # Debug first block
                    if d == fr_pivot.iloc[0]["date"] and efa == 1 and svc == "DC":
                        logging.info(f"DEBUG Block 1 DC: revenue={revenue:.2f}, deg={deg_cost:.2f}, boa={boa_opp_cost:.2f}, imb={imbalance_penalty:.2f}, vol={volatility_penalty:.2f}, net={net_margin:.2f}")
                elif svc == "ARBITRAGE":
...
def update_dashboard(schedule: pd.DataFrame, kpis: Dict[str, float]) -> None:
    ...
    ws.update(range_name=DASHBOARD_KPI_CELL, values=[[kpi_text]])
    ...
    ws.update(range_name=f"{chr(ord('A') + start_col - 1)}{start_row}", values=headers)
    ...
    ws.update(range_name=f"{chr(ord('A') + start_col - 1)}{start_row + 1}", values=rows)
