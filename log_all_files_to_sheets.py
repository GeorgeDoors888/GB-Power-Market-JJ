#!/usr/bin/env python3
"""
Log detailed file information from Google Drive folder to Google Sheets (with batch processing for large datasets)
"""
...
def get_file_details(service, folder_id, max_files=None):
    """Get detailed information about files in a Google Drive folder"""
    try:
        files_data = []
        page_token = None
        files_collected = 0
        ...
        if max_files:
            print(f"🔍 Collecting detailed information for up to {max_files} files...")
        else:
            print(f"🔍 Collecting detailed information for ALL files in the folder...")
        
        while max_files is None or files_collected < max_files:
            # Request detailed file information
            page_size = 1000
            if max_files is not None:
                page_size = min(1000, max_files - files_collected)
            ...
            for file_info in files:
                if max_files is not None and files_collected >= max_files:
                    break
...
def create_spreadsheet(sheets_service, title="Ofgem Files Analysis"):
...
def write_to_spreadsheet_batch(sheets_service, spreadsheet_id, files_data, batch_size=1000):
    """Write file data to the spreadsheet in batches to handle large datasets"""
    try:
        # Prepare headers
        headers = [
            'Document Title',
        ...
        print(f"📝 Writing {total_files} files to spreadsheet in batches of {batch_size}...")
        ...
        for batch_start in range(0, total_files, batch_size):
            ...
            print(f"📝 Writing batch {batch_start//batch_size + 1}: files {batch_start + 1} to {batch_end}")
            ...
            start_row = batch_start + 2  # +2 because of 1-indexed and header row
            end_row = start_row + len(batch_rows) - 1
            range_name = f'Sheet1!A{start_row}:P{end_row}'
            
            batch_body = {'values': batch_rows}
            
            # Retry logic for potential timeouts
            max_retries = 3
            for attempt in range(max_retries):
                try:
                    ...
                    print(f"  ✅ Batch completed: {result.get('updatedCells')} cells updated")
                    break
                    
                except Exception as e:
                    if attempt < max_retries - 1:
                        print(f"  ⚠️ Batch failed (attempt {attempt + 1}), retrying in 5 seconds: {e}")
                        time.sleep(5)
                    else:
        ...
        format_spreadsheet(sheets_service, spreadsheet_id, total_files + 1, len(headers))
...
def main():
    ...
    try:
        ...
        files_data = get_file_details(drive_service, folder_id, max_files=None)
        ...
        spreadsheet_title = f"Ofgem ALL Files Analysis - {timestamp}"
        ...
        success = write_to_spreadsheet_batch(sheets_service, spreadsheet_id, files_data)
        
        if success:
            print(f"\n🎉 Successfully logged {len(files_data)} files to Google Sheets!")
