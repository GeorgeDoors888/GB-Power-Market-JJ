#!/usr/bin/env python3
"""
Historical Data Loader with Schema Transformation (Debug Version)
...
It includes additional debug information.
"""
...
def get_file_content(blob):
    ...
    try:
        ...
        print_with_timestamp(f"Raw content: {content[:200]}...")  # Print first 200 chars for debugging
        return json.loads(content)
    except Exception as e:
        print_with_timestamp(f"Error reading {blob.name}: {str(e)}")
        return None

def get_bigquery_schema(table_ref):
    ...
    try:
        ...
        print_with_timestamp(f"Error getting schema for {table_ref}: {str(e)}")
        return None
...
def transform_demand_data(data):
    ...
    print_with_timestamp(f"Transforming demand data: {json.dumps(data, indent=2)}")
    ...
    for item in data['data']:
        ...
        print_with_timestamp(f"Transformed row: {json.dumps(row, indent=2)}")
...
def transform_frequency_data(data):
    ...
    print_with_timestamp(f"Transforming frequency data: {json.dumps(data, indent=2)}")
...
def transform_generation_data(data):
    ...
    print_with_timestamp(f"Transforming generation data: {json.dumps(data, indent=2)}")
...
def load_data_to_bigquery(rows, table_ref):
    ...
    # Get table schema for debug purposes
    schema = get_bigquery_schema(table_ref)
    print_with_timestamp(f"Target table schema: {schema}")
    
    # Print the first row for debugging
    print_with_timestamp(f"First row to load: {json.dumps(rows[0], indent=2)}")
...
def process_demand_data(bucket_name):
    ...
    for blob in blobs:
        print_with_timestamp(f"Found file: {blob.name}")
