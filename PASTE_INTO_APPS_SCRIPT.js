 ...
 * Called when "Refresh Dashboard" button is clicked OR when dropdown changes
 */
...
/**
 * onEdit trigger - Auto-refresh when dropdowns change
 * Automatically installed by Apps Script when you save
 */
function onEdit(e) {
  try {
    const range = e.range;
    ...
    // Only trigger on Dashboard sheet
    if (sheet.getName() !== "Dashboard") {
      return;
    }
    
    // Check if edited cell is one of the filter cells
    const row = range.getRow();
    const col = range.getColumn();
    
    // B3, D3, F3, I3, K3 (columns 2, 4, 6, 9, 11, row 3)
    if (row === 3 && [2, 4, 6, 9, 11].includes(col)) {
      Logger.log('Filter changed in cell ' + range.getA1Notation() + ' - refreshing dashboard');
      RefreshDashboard();
    }
  } catch (error) {
