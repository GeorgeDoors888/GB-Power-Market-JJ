#!/usr/bin/env python3
"""
Auto-ingest WINDFOR (Wind Forecast) from Elexon API
Runs every 15 min to backfill last 2 hours

Wind forecasts are critical for:
- Battery charging strategy (charge when wind is high = prices low)
- Grid balancing predictions
...
def get_latest_timestamp(client):
    ...
    try:
        query = f"""
        SELECT MAX(startTime) as latest
        ...
        if latest:
            logging.info(f"Latest WINDFOR data: {latest}")
            return latest
        else:
...
def fetch_windfor_data(from_dt, to_dt):
    """Fetch WINDFOR data from Elexon API"""
    ...
    logging.info(f"Fetching WINDFOR from {from_dt} to {to_dt}")
...
def upload_to_bigquery(client, records):
    ...
    for record in records:
        for field in ['startTime', 'publishTime', 'settlementDate']:
            if field in record:
    ...
    try:
        ...
        if errors:
            ...
            logging.info(f"✅ Successfully uploaded {len(records)} WINDFOR records")
            return True
    except Exception as e:
...
def main():
    ...
    logging.info("="*60)
    logging.info("WINDFOR Auto-Ingestion Starting")
    logging.info("="*60)
    ...
    # WINDFOR: Fetch from day after latest to now (can include partial current day)
    ...
    if from_dt > to_dt:
        logging.info(f"No new dates to fetch (latest: {latest.date()}, today: {to_dt.date()})")
        logging.info("="*60)
        return

    # Fetch data
    records = fetch_windfor_data(from_dt, to_dt)

    # Upload to BigQuery
    if records:
        upload_to_bigquery(client, records)
    else:
        logging.warning("⚠️ No WINDFOR data returned from API")

    logging.info("="*60)
    logging.info("WINDFOR Auto-Ingestion Complete")
    logging.info("="*60)
