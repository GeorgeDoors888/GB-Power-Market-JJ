#!/usr/bin/env python3
"""
Enhanced Data Exporter for Generation, BSUoS, and System Prices
================================================================

This script queries Google BigQuery for T_HUMR-1 generation, total gas
generation, and system prices, merges the data, and exports it to
both a CSV file and a Google Sheet.
"""
...
def create_and_populate_sheet(service, title, df):
    ...
    # Convert NaT to None (JSON serializable) and format other columns
...
def get_combined_data(client):
    ...
    # Query 1: T_HUMR-1 Generation
    humr1_query = f"""
    SELECT
        TIMESTAMP_TRUNC(timeTo, HOUR) + INTERVAL (IF(EXTRACT(MINUTE FROM timeTo) < 30, 0, 30)) MINUTE as datetime_utc,
    ...
    print("Fetching T_HUMR-1 generation data...")
    humr1_df = query_bigquery(client, humr1_query)

    print("\nFetching system price data...")
    price_df = query_bigquery(client, system_price_query)

    print("\nFetching total gas generation data...")
    ...
    price_df.set_index("datetime_utc", inplace=True)
    gas_df.set_index("datetime_utc", inplace=True)
...
def main():
    ...
    print("Starting Enhanced Data Export Process...")
    ...
    # Get combined data from BigQuery
    ...
    csv_filename = f"Enhanced_Generation_and_Price_Data_{timestamp}.csv"
    ...
    sheet_title = (
        f"Enhanced Generation and System Price Report - {datetime.date.today()}"
    )
