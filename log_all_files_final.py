#!/usr/bin/env python3
"""
Log ALL detailed file information from Google Drive folder to Google Sheets (with proper sheet sizing)
"""
...
def create_spreadsheet(sheets_service, title="Ofgem ALL Files Analysis", num_rows=10000):
    """Create a new spreadsheet with proper size"""
    try:
        # Determine appropriate number of rows (add buffer)
        required_rows = max(num_rows + 100, 2000)  # Add buffer and minimum
            ...
            'sheets': [{
                'properties': {
                    ...
                    'gridProperties': {
                        ...
                        'columnCount': 20  # We need 16 columns, so 20 is safe
                    }
                }
            }]
        }
        ...
        print(f"✅ Created spreadsheet with {required_rows} rows: https://docs.google.com/spreadsheets/d/{spreadsheet_id}")
...
def write_to_spreadsheet_in_batches(sheets_service, spreadsheet_id, files_data, batch_size=1000):
    ...
    try:
        ...
        print(f"📝 Writing {total_rows} rows in {total_batches} batches of {batch_size}...")
        ...
        for batch_num in range(total_batches):
            ...
            print(f"   📝 Writing batch {batch_num + 1}/{total_batches} (rows {start_idx + 1}-{end_idx})...")
            
            # Write batch to spreadsheet
            start_row = start_idx + 1  # 1-indexed
            range_name = f'Files Analysis!A{start_row}'
        ...
        print(f"✅ All {len(files_data)} files written to spreadsheet")
        
        # Format the spreadsheet
        num_rows = len(files_data) + 1  # +1 for header
...
def format_spreadsheet(sheets_service, spreadsheet_id, num_rows, num_cols):
    ...
    try:
        ...
        # Get the sheet ID (should be 0 for the first sheet, but let's be safe)
...
def main():
    ...
    try:
        ...
        # Create spreadsheet with appropriate size
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        spreadsheet_title = f"Ofgem ALL Files Complete Analysis - {timestamp}"
        spreadsheet_id = create_spreadsheet(sheets_service, spreadsheet_title, len(files_data))
