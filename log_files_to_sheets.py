#!/usr/bin/env python3
"""
Log detailed file information from Google Drive folder to Google Sheets
"""
...
def write_to_spreadsheet(sheets_service, spreadsheet_id, files_data):
    """Write file data to the spreadsheet"""
    try:
        # Prepare headers
        headers = [
            'Document Title',
        ...
        print(f"✅ {result.get('updatedCells')} cells updated in spreadsheet")
        
        # Format the spreadsheet
        format_spreadsheet(sheets_service, spreadsheet_id, len(rows), len(headers))
...
def main():
    ...
    try:
        ...
        success = write_to_spreadsheet(sheets_service, spreadsheet_id, files_data)
        
        if success:
            print(f"🎉 Successfully logged {len(files_data)} files to Google Sheets!")
