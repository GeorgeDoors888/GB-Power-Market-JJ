...
def run_dashboard(filters: Dict[str, Any]) -> None:
    ...
    # ---------------- BtM PPA Revenue (NEW) ----------------
    LOGGER.info("Calculating BtM PPA revenue...")
    btm_results, curtailment = get_btm_ppa_metrics(client)
    LOGGER.info("BtM PPA Total Profit: £%,.0f", btm_results["total_profit"])
    LOGGER.info(build_btm_ppa_summary_text(btm_results, curtailment))
