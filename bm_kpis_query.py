def get_bm_market_kpis(bq_client, PROJECT_ID='inner-cinema-476211-u9', DATASET='uk_energy_prod'):
    ...
    cashflows AS (
      SELECT
        ...
        SUM(CASE WHEN bidOfferAcceptanceType = 'OFFER' THEN cashflow ELSE 0 END) as offer_cashflow_gbp,
        SUM(CASE WHEN bidOfferAcceptanceType = 'BID' THEN ABS(cashflow) ELSE 0 END) as bid_cashflow_gbp,
    ...
    volumes AS (
      SELECT
        settlementPeriod as period,
        SUM(CASE WHEN acceptanceType = 'OFFER' THEN acceptedVolume ELSE 0 END) as offer_mwh,
        SUM(CASE WHEN acceptanceType = 'BID' THEN acceptedVolume ELSE 0 END) as bid_mwh,
