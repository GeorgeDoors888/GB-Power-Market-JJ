#!/usr/bin/env python3
"""
DNO Map OAuth Setup Script - Fixed UK-focused Version
...
5. Ensures the map focuses specifically on the UK
"""
...
DNO_DATA = [
    ...
    {
        "id": "SOUT",
        "name": "Scottish & Southern Electricity Networks (South)",
        "value": 14300,
    },
    ...
    {
        "id": "HECO",
        "name": "Scottish & Southern Electricity Networks (North)",
        "value": 8500,
    },
    {
        "id": "WSEB",
        "name": "Western Power Distribution (West Midlands)",
        "value": 12800,
    },
]
...
def main():
    ...
    script_id = create_apps_script_project(
        drive_service, apps_script_service, spreadsheet_id
    )
...
<body>
  ...
  <script>
    ...
    // Center map on Great Britain - UK bounds
    function focusOnGB() {
      if (!map) {
    ...
    function initializeMap() {
      try {
        ...
        // Define UK bounds for initial view
        var ukBounds = L.latLngBounds(
          [49.8, -8.6],  // Southwest corner
          [60.9, 1.8]    // Northeast corner
        );

        // Create the map centered on UK with appropriate zoom
        map = L.map('map', {
          // Use UK-specific center and appropriate zoom level
          center: [55.5, -3.0],  // Central UK coordinates
          zoom: 6,              // Higher zoom focused on UK
          minZoom: 5,
          maxZoom: 11
        });

        log("Map object created with UK-specific settings");
        ...
        try {
          ...
          // IMPORTANT: Immediately fit to the GeoJSON bounds to ensure UK focus
          try {
            var bounds = geojsonLayer.getBounds();
            log("GeoJSON bounds: " + JSON.stringify(bounds));

            // Use a tighter fit with less padding for UK-specific view
            ...
            // Enforce UK view if bounds are too wide (fallback)
            var boundsWidth = bounds.getEast() - bounds.getWest();
            log("Bounds width: " + boundsWidth);

            if (boundsWidth > 20) {
              log("Bounds too wide, forcing UK view");
              focusOnGB();
            }
    ...
    window.onload = function() {
      ...
      // First enable debug so we can see what's happening
      debugEnabled = true;
      var debugDiv = document.getElementById('debug');
      debugDiv.style.display = 'block';
      log("Debug panel enabled by default for troubleshooting");

      // Force UK focus after a short delay to ensure map is ready
      setTimeout(function() {
        initializeMap();

        // Double-check UK focus after map initialization
        setTimeout(function() {
          focusOnGB();
          log("Applied secondary UK focus");
        }, 2000);
      }, 500);
      ...
      var visibilityCheck = setInterval(function() {
        ...
        if (mapContainer && mapContainer.clientWidth > 0 && mapContainer.clientHeight > 0) {
          ...
          if (map) {
            map.invalidateSize();
            focusOnGB(); // Re-focus whenever size changes
            log("Map size invalidated and UK focus reapplied");
          }
    ...
    if script_id != "MANUAL_SETUP_REQUIRED":
        try:
            ...
            print(
                "You may need to manually create the Apps Script project and add the files."
            )
    ...
    if script_id != "MANUAL_SETUP_REQUIRED":
        print(
            "Apps Script Editor URL: https://script.google.com/d/" + script_id + "/edit"
        )
    ...
    if not credentials or not credentials.valid:
        if credentials and credentials.expired and credentials.refresh_token:
            ...
            if not os.path.exists("client_secrets.json"):
                print(
                    "Error: client_secrets.json not found. Please download this file from Google Cloud Console."
                )
                return None
...
def create_or_update_spreadsheet(sheets_service, drive_service):
    ...
    if not spreadsheet_id:
        try:
            ...
            spreadsheet = {
                "properties": {"title": "UK DNO Map Data"},
                "sheets": [{"properties": {"title": "DNO_Data"}}],
            }
...
def create_apps_script_project(drive_service, apps_script_service, spreadsheet_id):
    ...
    if not script_id:
        try:
            ...
            request = {"title": "UK DNO Map", "parentId": spreadsheet_id}
