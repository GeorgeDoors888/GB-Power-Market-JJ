...
PRE_COMMIT_BIN = (
    VENV_DIR / ("Scripts/pre-commit.exe" if IS_WIN else "bin/pre-commit")
).as_posix()
...
def install_vscode_extensions():
    ...
    if not code_cli:
        print(
            "VS Code CLI 'code' not found; skipping auto-install. "
    ...
    for ext in ext_ids:
        run(
            f"{shlex.quote(code_cli)} --install-extension {shlex.quote(ext)} --force",
            check=False,
        )
...
def ensure_ssh():
    ...
    if not KEY_PATH.exists():
        ...
        run(
            f'ssh-keygen -t ed25519 -C {shlex.quote(GIT_EMAIL)} -f {shlex.quote(KEY_PATH.as_posix())} -N ""'
        )
    block = [
        "Host github.com",
        "  HostName github.com",
    ...
    if IS_MAC:
        ...
        run(
            'eval "$(ssh-agent -s)" && ssh-add ' + shlex.quote(KEY_PATH.as_posix()),
            check=False,
        )
...
def scaffold_ai_memory():
    ...
    (AI_DIR / "index_repo.py").write_text(
...
""",
    ...
    os.chmod(
        (AI_DIR / "index_repo.py").as_posix(),
        stat.S_IRUSR | stat.S_IWUSR | stat.S_IXUSR,
    )

    (AI_DIR / "search_memory.py").write_text(
        r"""#!/usr/bin/env python3
...
""",
    ...
    os.chmod(
        (AI_DIR / "search_memory.py").as_posix(),
        stat.S_IRUSR | stat.S_IWUSR | stat.S_IXUSR,
    )
...
""",
    ...
    # Gitleaks
    (ci_dir / "gitleaks.yml").write_text(
        """name: gitleaks
...
""",
    ...
    # EditorConfig
    (WS_ROOT / ".editorconfig").write_text(
        """root = true
[*]
end_of_line = lf
...
""",
    ...
    (WS_ROOT / ".pre-commit-config.yaml").write_text(
        """repos:
